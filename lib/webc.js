...
exports.watch = function(options, callback) {
  ...
  options.extension = function(inputPath, outputPath) {
    ...
    var watcher = fs.watch(inputPath, function(event, filename) {
      ...
      if (filename && fs.lstatSync(inputPath).isDirectory()) {
        sourcePath = sourcePath + '/' + filename
        targetPath = targetPath + '/' + filename
      }

      // Somehow fs.watch sometimes sends multiple alerts for the same file. This makes sure, it is treated only once.
      var now = Date.now();
      if (lastChangeFile === sourcePath && lastChangeTime > now - 500) { return; }
      lastChangeFile = sourcePath;
      lastChangeTime = now;
...
function isInIgnore(path, options) {
  for (var i = 0; i < options.ignore.length; i++) {
    if (path.startsWith(options.ignore[i])
       || path.endsWith(options.ignore[i])) {
      return true;
    }
  };
  return false;
